...
> 针对 [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) 官方连接 DeepSeek API 的方式 + 官方公众号收录的 [dsh-TUI](https://github.com/ccch1mneyyy/dsh-TUI) 终端界面（v0.9+，低资源占用），做成的 Windows 一键安装/配置脚本。
...
| 2. 安装 Harness + TUI | `npm install -g @deepseek-ai/dsh @deepseek-harness-tui/dsh-tui`（官方 CLI + TUI 前端） |
| 3. 安装 pnpm | 需要 `>=10`（`dsh plugin` 装 profile 依赖用；pnpm 9 会启动即退，见上游 issue #60） |
| 4. 创建 TUI profile | `dsh plugin --profile dsh-tui add @deepseek-harness-tui/dsh-tui@latest`（`dsh-tui` 命令 = `dsh --profile dsh-tui`） |
...
1. **升级到最新版**：安装 `@deepseek-harness-tui/dsh-tui`（当前 v0.9.x），上游在新版中主打「低资源占用、长会话稳定可靠」，并修复了大量渲染与响应性问题（如 ESC 打断卡死等），体验与旧版是两代差距。
...
powershell -ExecutionPolicy Bypass -File install.ps1 -Headless `
  -ApiKey "sk-xxxxxxxx" -BaseUrl "https://api.deepseek.com" -ReasoningEffort high -Model deepseek-v4-pro -SmoothMode $true
```

常用参数：`-ConfigureOnly`（只写配置）、`-NoShortcut`、`-NoLaunch`、`-SkipNodeCheck`、`-SkipNpmInstall`、`-SkipTuiSetup`、`-NoValidate`、`-SmoothMode:$false`（关闭流畅模式）、`-DshHome <路径>`（自定义家目录，测试用）。
...
- **想升级 TUI？** 重跑 `install.bat`（会 `add @latest`），或 TUI 内执行 `/update`；全局命令用 `npm install -g @deepseek-harness-tui/dsh-tui@latest`。
